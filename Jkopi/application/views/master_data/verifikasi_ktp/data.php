...
<script>
	...
	function getData() {
		$.ajax({
			url: "<?php echo base_url() . "master_data/Verifikasi_KTP/getAllData" ?>",
			dataType: "json",
			beforeSend: function() {
			...
			success: function(data) {
				...
				for (var i = 0; i < data.length; i++) {
					baris += '<tr>' +
						...
						'<td>' + (data[i].verif_ktp == 1 ? '<div class="badge badge-warning badge-pill px-3">Proses Permintaan</div>' :
							data[i].verif_ktp == 0 ? '<div class="badge badge-danger badge-pill px-3">Belum Verifikasi</div>' :
						...
						'<td> <a href="Verifikasi_KTP/detail/' + data[i].id_pengguna + '" class="btn btn-datatable btn-icon btn-transparent-dark"><i class="fa fa-search-plus "></i></a></td>'
					'</tr>'
				}
